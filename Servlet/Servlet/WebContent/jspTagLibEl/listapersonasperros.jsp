...
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    ...
    <%@ page import="ejemploval.Persona, java.util.List, ejemploval.Perro" %>
...
lista_personas_perros.add(new Persona(31, "Tamara",new Perro("Yako", "Golden")));
session.setAttribute("lista_personas_perros", lista_personas_perros);
%>

<br>PRIMER FORMA, con puntos, recorrido tipo Iterator<br>
<c:forEach items="${lista_personas_perros}" var="persona"><%--la persona equivale a un auxiliar y lo guarda en el auxiliar. --%>
	<c:out value="${persona.nombre}"></c:out> de
	<c:out value="${persona.edad}"></c:out> años,  es dueño de
	<c:out value="${persona.perro.nombre}"></c:out> de raza<%--con esto va a la clase persona y recoge el getnombre y a su vez va a la calse perro y recoge el getnombre y el getraza--%>
